from etl_10k.config import RAW_LM_DICT_DIR
import datetime as dt
from collections import Counter
import re
import sys

def load_masterdictionary(print_flag=False, f_log=None, get_other=False):
    file_path = RAW_LM_DICT_DIR / "Loughran-McDonald_MasterDictionary_1993-2024.csv"
...
def lm_tone(toks: str):
    (
    _, _,
    sentiment_categories,
    sentiment_dictionaries,
    _, _
    ) = load_masterdictionary(get_other=True)
    ...
    for cat in sentiment_categories:
        cat_count = sum(counts[w] for w in counts if w in sentiment_dictionaries[cat])
        out[cat] = cat_count
